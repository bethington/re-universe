...
validate_security() {
    if [[ -f "$ENV_FILE" ]]; then
        if grep -q "CHANGE_ME_SECURE_PASSWORD\|***REDACTED***\|bsim_password" "$ENV_FILE"; then
...
show_connection_info() {
    ...
    echo "  URL:      postgresql://ben:***REDACTED***@localhost:5432/bsim"
    echo ""
    echo "Ghidra BSim Connection:"
    echo "  1. Open Ghidra → Tools → BSim Search"
    echo "  2. Server: postgresql://ben:***REDACTED***@localhost:5432/bsim"
