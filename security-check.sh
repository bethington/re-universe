...
check_default_credentials() {
    ...
    run_security_check "No hardcoded ben:goodyx12 credentials" \
        "! grep -r 'ben:goodyx12' . --include='*.sh' --include='*.md' --include='*.yml' 2>/dev/null" \
        "critical"
...
show_security_recommendations() {
    ...
    if [[ $FAIL_COUNT -gt 0 ]]; then
        ...
        if grep -r "ben:goodyx12" . --include="*.sh" --include="*.md" --include="*.yml" 2>/dev/null; then
            echo "🔴 CRITICAL: Remove all hardcoded credentials (ben:goodyx12)"
            echo "   → Run: sed -i 's/ben:goodyx12/[username]:[password]/g' \$(find . -name '*.md' -o -name '*.sh')"
        fi

        if [[ -f ".env" ]] && grep -q "CHANGE_ME\|goodyx12\|bsim_password" .env; then
